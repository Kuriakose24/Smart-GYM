...
# Video input
# -------------------------------
cap = cv2.VideoCapture("dataset/videofiles/raw_data/raw_data/data-crawl/push-up/push-up_5.mp4")
# webcam option
...
while cap.isOpened():
    ...
    if result.pose_landmarks:
        ...
        cv2.putText(frame,f"Stage: {stage}",
                    (30,120),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2)

        cv2.putText(frame,f"Posture: {posture}",
                    (30,160),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,255),2)

        cv2.putText(frame,f"Feedback: {feedback}",
                    (30,200),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)

        cv2.putText(frame,f"Elbow: {int(elbow_angle)}",
                    (30,240),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,0),2)

        cv2.putText(frame,f"Back: {int(back_angle)}",
                    (30,270),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,0),2)

        cv2.putText(frame,f"Hip: {int(hip_angle)}",
                    (30,300),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,0),2)
